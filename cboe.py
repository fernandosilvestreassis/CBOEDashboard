            ...
            with tab3:
                ...
                with tab4:
                    ...
                    with tab7:
                        ...
                        st.header(f"{ticker.symbol}"' Total Open Interest by Strike')
                        chart_total_interest_data = pd.DataFrame(columns = ['TotalOpenInterest'])
                        put_oi = ticker.by_strike['Put OI']*(-1)
                        call_oi = ticker.by_strike['Call OI']
                        net_oi = call_oi + put_oi
                        chart_total_interest_data.TotalOpenInterest = net_oi
                        st.bar_chart(
                            chart_total_interest_data,
                            y=['TotalOpenInterest'],
                            width=0,
                            height=600,
                            use_container_width = True,
                    ...
                    with tab8:
                        ...
                        st.write('\n')
                        st.header(f"{ticker.symbol}"' Total Open Interest by Expiration')
                        chart_total_open_interest_expiration_data = pd.DataFrame(columns = ['TotalOpenInterest'])
                        chart_total_open_interest_expiration_data.TotalOpenInterest = ticker.by_expiration['Call OI'] + ticker.by_expiration['Put OI']*(-1)
                        chart_total_open_interest_expiration_data.fillna(axis = 1, value = 0, inplace = True)
                        st.bar_chart(
                            chart_total_open_interest_expiration_data,
                            y=['TotalOpenInterest'],
                            width=0,
                            height=600,
                            use_container_width=True,
                ...
                with tab5:
                    ...
                    with tab7:
                        ...
                        st.header('Total Nominal Gamma Exposure Per 1% Change in 'f"{ticker.symbol}")
                        chart_total_gamma_data = pd.DataFrame(columns = ['TotalGamma'])
                        chart_total_gamma_data.TotalGamma = ticker.by_strike['Net GEX']
                        chart_total_gamma_data.fillna(axis = 1, value = 0, inplace = True)
                        st.bar_chart(
                            chart_total_gamma_data,
                            y= ['TotalGamma'],
                            use_container_width = True,
                            width=0,
                            height=600
                    ...
                    with tab8:
                        ...
                        st.header('Total Nominal Gamma Exposure per 1% Change in 'f"{ticker.symbol}")
                        chart_total_gamma_expiration_data = pd.DataFrame(columns = ['TotalGamma'])
                        put_gamma_ex = ticker.by_expiration['Put GEX']*(-1)
                        call_gamma_ex = ticker.by_expiration['Call GEX']
                        chart_total_gamma_expiration_data.TotalGamma = call_gamma_ex + put_gamma_ex
                        chart_total_gamma_expiration_data.fillna(axis = 1, value = 0, inplace = True)
                        st.bar_chart(
                            chart_total_gamma_expiration_data,
                            y=['TotalGamma'],
                            use_container_width = True,
                            width=0,
                            height=600
                ...
                with tab6:
                    st.write('\n')
                    tab9,tab10,tab11,tab12 = st.tabs(["Skew", "Smile", "Surface", "Implied Volatility"])
                    with tab9:
                    ...
                    with tab10:
                        ...
                        st.subheader("Historical 30-Day IV "f"{ticker.symbol}")                
                        st.line_chart(
                            chart6_data,
                            y = ['Call IV', 'Put IV'],
                            height = 450,
                    ...
                    with tab12:
                        # Obtendo os dados do SPY usando yfinance
                        ticker = "SPY"
                        ...
                        # Calculando a volatilidade implícita de 30 dias
                        data["IV_30"] = data["Close"].pct_change().rolling(window=30).std() * (252 ** 0.5) * 100

                        # Criando um gráfico interativo usando o Streamlit
                        st.line_chart(data["IV_30"])

                        start_date = pd.to_datetime("now") - pd.DateOffset(years=1)
                        end_date = pd.to_datetime("now")
                        data = yf.download(ticker, start=start_date, end=end_date)

                        # Calculando a volatilidade histórica de 30 dias
                        data["Vol_30"] = data["Close"].pct_change().rolling(window=30).std() * (252 ** 0.5) * 100

                        # Criando um gráfico interativo usando o Streamlit
                        st.line_chart(data["Vol_30"])
    else:
        pass
